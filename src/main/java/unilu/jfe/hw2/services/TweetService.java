...
/**
 * Class responsible for implementing all manipulations to be done with Tweets
 * as well as with Feedbacks.
 * Additionally, all the manipulations linked with redis and Tweets are
...
public class TweetService {
    ...
    public Tweet postTweet(String userId, String title, String message, int validityLength) {
        ...
        Tweet newTweet = new Tweet(tweetId, title, message, userName, userId, LocalDate.now().toString(),
                validityLength, new ArrayList<>());

        // Adding the tweet to the db
    ...
    public List<Tweet> searchFor(String word) {
        ...
        for (String key : rt.keys("TWEET[0-9]*")) {
            ...
            // General search
            if (tempTweet.getId().contains(word) || tempTweet.getAuthorName().contains(word)
                    || tempTweet.getId().contains(word) ||
                    tempTweet.getMessage().contains(word) || tempTweet.getPostDate().contains(word)
                    || tempTweet.getTitle().contains(word)) {
